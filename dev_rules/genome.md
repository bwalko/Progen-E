...
2. **`genome_composite_scores`** — Numeric ratings from `config/genome_composite_ratings.csv` for browser/report sorting. These use nonlinear component curves in `library.genome_composites`: individual component fits are curved, blended with a weighted geometric-style mean, and damped by overall coherence so a single high component cannot carry the whole rating. Scores are lower-bounded at `0` and intentionally not upper-clamped: `1.0` is the normal max, while rare all-aligned outliers may exceed it.
3. **`genome_trait_phrases`** — Single-trait callouts from `library.personality_interpreter.interpret_genome_personality` using the prototype defaults (`DEFAULT_PERSONALITY_Z_CUTOFFS = (1.0, 2.0, 2.5, 3.0)`, `sigma = DEFAULT_GENOME_MAGNITUDE_STDEV`, `reference_magnitude = 50.0`). Only **extremes** are recorded: traits inside the ordinary band (`z ≤ c0`) produce no entry. Phrases use the `*description` columns from `config/genome.csv` and read as "\<name\> is \<phrase\>". Adverb intensifiers at higher z bands are: bare label → `very` → `extremely` / `remarkably` (toward dysfunction / toward ideal) → `clinically` / `incredibly` (top tier, "mental-illness-level" extremes vs. uncanny excellence).
...
Numeric composite score visibility is age-gated. The raw rating formulas can be recomputed at any time, but code and UI that present or act on known scores should use the helper functions in `library.genome_composites` so only revealed ratings are returned. Two ratings become known at each age from birth: age `0` reveals `make_friends`, `force_get_way_desire`; age `1` reveals `make_enemies`, `isolation_preference`; age `2` reveals `convince_people`, `insanity`; age `3` reveals `good_done_desire`, `creative_intellect`; age `4` reveals `psychopathy`, `revenge_desire`; age `5` reveals `disguise_motive`, `lie_or_cheat_willingness`; age `6` reveals `ruthless_ambition`, `lead_others_ability`; age `7` reveals `evil_done_desire`, `practical_intellect`; age `8` reveals `honest_work_desire`, `physical_strength`; age `9` reveals `enrich_self_desire`, `sexual_object`; age `10` reveals `sexual_magnetism`.
...
- `config/genome_composite_ratings.csv` — numeric composite rating recipes.
- `library/person.py` — `Person.genome`, `Person.genome_composite_names`, `Person.genome_composite_scores`, `Person.genome_trait_phrases`.

...
- `library/event_history_report.py`
  - Event-history tuning reports: counts by type/year, event-memory visibility counts, tracked incident counts including zeros, numeric payload metrics, and public chronicle prose samples.
...
- `library/simulation_trade_networks.py`
  - Annual maritime mercantile layer. Scores active settlements as port-network hubs from geography, sea-route centrality, trade/admin/transport/craft/knowledge job density, settlement prosperity/market pull, and persisted portable knowledge domains (`navigation`, `shipbuilding`, `writing`, `accounting`, `trade_law`, `craft`, `art`). High-scoring coastal hubs can found capped commercial outposts via sea routes, moving founder households with spouse/partner and dependent minors; mature outposts can autonomize, and weakened roots can recenter the trade network on a stronger active member.
